      ...
      <div class="seller-view">
        ...
        <div class="pics-wrapper" ref="picsView">
          <ul class="pics-list" v-if="seller.poi_env" ref="picsList">
            <li class="pics-item" v-for="(imgUrl,index) in seller.poi_env.thumbnails_url_list" :key="index" ref="picItem">
              <img :src="imgUrl" alt="">
            </li>
          </ul>
        </div>
      ...
      if(response.code==0){
        ...
        this.$nextTick(()=>{
          if(this.seller.poi_env.thumbnails_url_list){
            let imgWidth=this.$refs.picItem[0].clientWidth
            let mlWidth=11
            let width=(imgWidth+mlWidth)*this.seller.poi_env.thumbnails_url_list.length
